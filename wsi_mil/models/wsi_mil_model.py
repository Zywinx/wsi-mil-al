...
class WSIBaselineMIL(nn.Module):
    def __init__(
        ...
        self.encoder = TileEncoder(out_dim=embed_dim, pretrained=encoder_pretrained, dropout=encoder_dropout)
        self.mil = AttentionMIL(in_dim=embed_dim, attn_dim=attn_dim, dropout=mil_dropout)
    ...
    def forward(self, bag_imgs: torch.Tensor) -> ForwardOut:
        ...
        return ForwardOut(slide_logit=slide_logit, slide_prob=slide_prob, alpha=alpha, h=h, tile_z=z)